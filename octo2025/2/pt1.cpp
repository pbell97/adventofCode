...
void PuzzleSolution(vector<string> input, vector<string> arguments) {
    ...
    for (const auto& line : input) {
        ...
        try {
            nums.push_back(iNum(stoul(split[0]), stoi(split[1].substr(2))));
    ...
    for (uint32_t candidate : all_candidates) {
        int total_error = 0;
    ...
    for (const auto& num : nums) {
        ...
        cout << "Expected: " << num.BitsAway << ", Actual: " << hamming_distance;
