...
struct Machine {
    uint8_t DesiredBitOutCome;
    vector<vector<uint8_t>> Buttons;
    vector<uint8_t> ButtonActions;
    vector<uint8_t> Joltages;

    Machine(uint8_t desiredBitOutCome, vector<vector<uint8_t>> buttons, vector<uint8_t> buttonActions,
            vector<uint8_t> joltages)
...
void generatePermutations(const vector<uint8_t>& nums, int maxLength, vector<uint8_t>& current,
...
vector<vector<uint8_t>> getAllPermutations(const vector<uint8_t>& nums, int maxLength) {
    vector<vector<uint8_t>> result;
...
void PuzzleSolution(vector<string> input, vector<string> arguments) {
    ...
    for (const auto& machine : machines) {
        vector<vector<uint8_t>> perms = getAllPermutations(machine.ButtonActions, 10);
        int buttonPushes = 0;
