    ...
    do {
        ...
        for (int rowNum = 0; rowNum < input.size(); rowNum++) {
            ...
            for (int colNum = 0; colNum < row.size(); colNum++) {
                ...
                adjacentRolls +=
                    (upperRow != "" && colNum != 0 && upperRow.at(colNum - 1) == '@') ? 1 : 0;  // UpperLeft
                adjacentRolls += (upperRow != "" && colNum != (row.size() - 1) && upperRow.at(colNum + 1) == '@')
                                     ? 1
                                     : 0;  // UpperRight
                adjacentRolls +=
                    (lowerRow != "" && colNum != 0 && lowerRow.at(colNum - 1) == '@') ? 1 : 0;  // LowerLeft
                adjacentRolls += (lowerRow != "" && colNum != (row.size() - 1) && lowerRow.at(colNum + 1) == '@')
                                     ? 1
                                     : 0;  // LowerRight

                runningTotal += adjacentRolls < 4 ? 1 : 0;
                if (adjacentRolls < 4) {
                    input[rowNum].at(colNum) = '.';
                }
            }
        }
        totalRolls += runningTotal;
