        ...
        # Conjunction (Consider building the list before hand...might need to and set to low)
            ...
            # Updates it memory when receives
                ...
                # Otherwise it sends out a high pulse
                    ...
                    if self.name == "jq" and any([x == "high" for x in self.inputMemory.values()]) and self.inputMemory != self.currentjq:
                        self.currentjq = self.inputMemory.copy()
                        print(f"jq map: {self.inputMemory} {buttonCount}")
                    for connection in self.connections:
                        self.masterQueueRef.append({"target": connection, "signalType": "high", "sendingName": self.name})
...
# &jq is what sends pulses to rx, all the inputs to jq need to be high to send a low pulse to rx
    # Figure out what the outputs of jq are